...
|---------|:----:|:--------:|---------------|-----------------|:-----------------:|---------------|
| **Flink** (first‑class) | JVM/Py | streamed | native keyBy | native checkpoints | **yes, runner²** | full framework |
...
| **Pekko** | JVM | online/actor | actor mailbox | Persistence | engine Runtime⁴ | ActorTestKit yes |
| **Clojure** (first‑class) | Clojure | online | per‑conv serialize | **Datomic** (immutable) | **yes, EDN+YAML** | runs live yes |
| **Temporal** | JVM/Go | online | one exec/id | event‑sourced | engine Runtime⁴ | testsuite yes |
| **Pulsar Functions** | JVM | streamed | Key_Shared | BookKeeper state | engine Runtime⁴ | in‑mem ctx yes |
| **Ray** | Python | online/actor | actor | external | core build³ | import-guarded |
| **NATS JetStream** | Py/Go | online | subject+CAS | KV + stream | **yes, local+nats** | runs live yes |
...
| **Celery** | Python | online | queue+lock | acks+store | **yes** | runs live yes |
| **Dask** | Python | batch |, | retry | core build³ | runs live yes |
| **Airflow** | Python | batch/sched |, | retry/idempotent | core build³ | simulate yes |
| **Local** (in‑proc) | all 3 | online | per‑key lock | in‑mem/Redis | **yes, all 3** | runs live yes |
