...
> `spec/tools/conformance_matrix.py` from a run of the 15 shared agentic/v1 conformance fixtures
...
> feature *coverage and delivery model* per core/backend and are maintained by hand.
...
guardrails, listeners, multi-turn memory, agent-calls). Which of those a given runtime
has actually *proven* against the shared fixtures is [`../capabilities.md`](../capabilities.md);
the columns below capture the *real* per-backend differences in delivery model.
