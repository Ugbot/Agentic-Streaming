...
```bash
mvn -f ports/experimental/temporal/pom.xml compile            # BUILD SUCCESS
mvn -f ports/experimental/temporal/pom.xml test               # 2 tests (banking + extended-graph via worker factory)
mvn -f ports/experimental/temporal/pom.xml -q compile exec:java   # runs the banking demo on an in-memory Temporal service
# ->
